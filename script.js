...
let isLoadingData = false; // CỜ CHỐNG VÒNG LẶP ĐỒNG BỘ
...
function saveData() {
  if (isLoadingData) return; // Không lưu khi đang trong quá trình load
...
function loadData(externalData = null) {
  isLoadingData = true; // Bật cờ chặn save loop
  ...
  if (raw) {
    try {
      ...
      if (state.semesters && state.semesters.length > 0) {
        // Tự động cắt bỏ các kỳ trùng, giữ lại đúng 9 kỳ gốc duy nhất
        const cleanSemesters = state.semesters.slice(0, 9);
  ...
  calculate();
  isLoadingData = false; // Tắt cờ sau khi nạp xong
  saveData(); // Ghi đè lại dữ liệu sạch chuẩn 9 kỳ (157 TC)
}
  ...
  setTimeout(() => {
    msgContainer.innerHTML += `
      <div class="bg-indigo-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-2.5 rounded-xl max-w-[85%] border border-indigo-100 dark:border-slate-700">
        Tracy đã nhận câu hỏi! CPA hiện tại của bạn là <b>${cpa}</b>. Hãy ưu tiên học cải thiện các môn có điểm D/D+ để kéo CPA nhanh nhất nhé!
      </div>
    `;
